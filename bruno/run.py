...
from bruno.brain.request_classifier import is_likely_real_request
...
# When Bruno is awake we listen longer so the user can say long requests (Alexa-style)
voice_listen_seconds = [4.0]   # [0] = 10.0 when awake, 4.0 when not; worker reads each cycle

def voice_worker():
    global latest_transcript
    while True:
        sec = voice_listen_seconds[0]
        text = listen_and_transcribe(sec)
        if text:
            with voice_lock:
...
def _request_after_wake_word(transcript: str, wake_word: str) -> str:
    """Return everything after the wake word so it's processed immediately. Handles 'Bruno, what's the weather' or 'Hey Bruno what time is it'."""
    t = (transcript or "").strip()
    if not t:
        return ""
    low = t.lower()
    i = low.find(wake_word)
    if i < 0:
        return ""
    rest = t[i + len(wake_word):].strip().lstrip(",").strip()
    return rest
...
def main():
    ...
    # Alexa-style: wake word "Bruno", then listen for request (longer window + longer recording when awake)
    bruno_awake_until = 0.0
    WAKE_WORD = "bruno"
    WAKE_WINDOW_SEC = 60.0   # generous window after "Bruno" so user can phrase long requests
    LISTEN_SEC_AWAKE = 12.0  # record up to 12s when awake to capture long requests
    LISTEN_SEC_IDLE = 4.0
    identity_greeted_names = set()
    ...
    while True:
        ...
        frame_count += 1
        now = time.time()
        bruno_awake = now <= bruno_awake_until
        spoke_from_voice_this_frame = False

        # Tell voice worker to use longer recording when we're in the wake window (long requests)
        voice_listen_seconds[0] = LISTEN_SEC_AWAKE if bruno_awake else LISTEN_SEC_IDLE
        ...
        if transcript:
            ...
            # ---------------- WAKE-UP: "Bruno" — everything immediately after is the request ----------------
            request_text = None
            if WAKE_WORD in normalized:
                bruno_awake_until = now + WAKE_WINDOW_SEC
                bruno_awake = True
                if normalized == WAKE_WORD:
                    speak("Yes?")
                else:
                    # Same utterance: "Bruno, what's the weather" -> process "what's the weather" now
                    request_text = _request_after_wake_word(transcript, WAKE_WORD)
            elif bruno_awake and normalized != WAKE_WORD:
                request_text = transcript

            # ---------------- PROCESS REQUEST (same turn after "Bruno" or follow-up in window) ----------------
            if request_text:
                bruno_awake_until = now + WAKE_WINDOW_SEC
                req_norm = request_text.lower().strip()

                if "check my skin" in req_norm:
                    ...
                    if brain_out and brain_out.get("say"):
                        speak(brain_out["say"])
                        spoke_from_voice_this_frame = True

                elif "check my heart rate" in req_norm:
                    face_box = (
                ...
                elif is_likely_real_request(request_text):
                    face_box = (
                        last_face_matches[0]["bbox"]
                        if last_face_matches else None
                    )
...
# Background Pose
        ...
        if autopilot_enabled and bruno_awake and not spoke_from_voice_this_frame:
            autopilot_out = autopilot.decide(state, risk)
            if autopilot_out and autopilot_out.say:
                speak(autopilot_out.say)

        # --- Primary identity speech: only when awake, and only first time we see each person ---
        if bruno_awake:
            now = time.time()
            primary_name = None
            best_area = -1
            ...
            if primary_name and primary_name != last_spoken["name"]:
                if now - last_spoken["t"] >= SPEAK_COOLDOWN_SEC and primary_name not in identity_greeted_names:
                    speak(f"Hey {primary_name}.")
                    last_spoken = {"t": now, "name": primary_name}
                    identity_greeted_names.add(primary_name)
                else:
                    last_spoken = {"t": now, "name": primary_name}
